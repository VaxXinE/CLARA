...
previous: "87-AI-Gateway-Architecture.md"
next: "89-Prompt-Template-Management.md"
project: "CLARA"
---

# Model Provider Abstraction

> *"Defines how CLARA should abstract model providers, model selection, retries, provider configuration, and future provider switching."*
...
Hard-coding one provider into product logic makes future model changes, fallback, cost management, and testing harder.
...
CLARA should implement provider abstraction so product features depend on AI capabilities, not hard-coded vendor-specific calls.
...
interface AIProvider {
  ...
  generateStructured<T>(request: AIStructuredRequest): Promise<T>;
}
```
...
Do not log raw API keys or secret provider config.

---

# Fallback Strategy

For MVP, fallback can be simple:

```text
Show safe failure message
Allow manual workflow to continue
Do not auto-retry high-cost requests aggressively
```
