...
previous: "122-Integration-Permissions-and-Governance.md"
next: "124-Integration-Release-and-Rollout-Strategy.md"
project: "CLARA"
---

# Integration Security Testing

> *"Defines security test strategy for webhooks, provider payloads, credentials, SSRF, replay attacks, rate limits, and cross-tenant integration access."*
...
Integration vulnerabilities often come from trusting external payloads or mishandling secrets.
...
CLARA integration implementation must include tests for untrusted input, invalid signatures, replayed events, duplicate events, unauthorized access, and secret leakage.
...
# Integration Security Test Cases
...
secret values not returned in API response
raw credential not logged
rate limit enforced for ingestion endpoint
SSRF prevention for outbound webhook URLs
large payload rejected
malformed attachment metadata rejected
```
...
Outbound webhook URLs must be validated.

Block or protect access to:
