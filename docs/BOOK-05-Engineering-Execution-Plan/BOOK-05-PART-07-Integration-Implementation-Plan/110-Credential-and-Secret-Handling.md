...
previous: "109-Connector-Lifecycle-Implementation.md"
next: "111-Inbound-Webhook-Ingestion.md"
project: "CLARA"
---

# Credential and Secret Handling

> *"Defines implementation plan for OAuth tokens, API keys, webhook signing secrets, provider credentials, and secret references."*
...
Integration credentials are high-risk. Leaked tokens can expose customer messages, provider accounts, and business systems.
...
CLARA should store only credential metadata and secret references in normal database tables, while raw secrets live in secure runtime or secret manager storage.
...
Normal database tables should store:
...
Raw values should live in:

```text
secret manager
encrypted vault
runtime environment
provider-managed OAuth storage
```
...
- Never return raw credential values to UI.
- Never log tokens or API keys.
- Rotate/revoke where practical.
