...
multi-channel registry and channel account read foundation now exposes safe Gmail capabilities, Webchat inbound plus simulated human-triggered Webchat reply/status visibility, WhatsApp official inbound webhook materialization, and planned Instagram/TikTok metadata
```
...
multi-channel support is registry/account/health metadata plus Webchat inbound, simulated Webchat reply/status visibility, and WhatsApp official inbound webhook materialization outside Gmail; WhatsApp still has no outbound send/template/media/contact sync runtime, and Instagram/TikTok have no provider integration yet
```
