...
gmail oauth connect and callback completion wiring now exist for disabled/simulated local flows, a real token exchange client boundary now exists with mocked tests only, and a Gmail API client boundary now exists behind explicit config, but there is still no public real completion flow, Gmail profile resolution, refresh lifecycle, inbound fetch, or outbound Gmail send
