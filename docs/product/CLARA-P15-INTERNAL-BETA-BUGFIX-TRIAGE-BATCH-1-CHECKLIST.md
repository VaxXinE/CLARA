# CLARA P15 Internal Beta Bugfix Triage Batch 1 Checklist
...
- Confirm each issue has safe reproduction steps.
- Confirm no issue evidence includes secret, token, cookie, auth header, raw DOM,
  raw HTML, raw prompt, raw provider payload, raw webhook payload, or payment
  data.
- Confirm any accepted bugfix preserves AuthContext and workspace membership as
  the authorization source of truth.
- Confirm unresolved issues are moved to known issue disposition.
