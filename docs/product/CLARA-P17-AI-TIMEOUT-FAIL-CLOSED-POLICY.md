# CLARA P17 AI Timeout Fail-Closed Policy

P17 Real AI Analysis Activation is current.
P17-PR-01 is current.

AI timeout policy is required. Invalid timeout configuration fails closed.
Configured provider calls in later PRs must use bounded timeouts and safe
sanitized provider error responses.

P17-PR-01 prepares AI provider runtime configuration but does not yet execute extension snapshot AI analysis.
