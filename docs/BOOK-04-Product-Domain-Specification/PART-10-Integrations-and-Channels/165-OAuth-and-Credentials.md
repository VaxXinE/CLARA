...
previous: "164-Connector-Lifecycle.md"
next: "166-Webhook-Ingestion.md"
---

# OAuth and Credentials

> *"Defines product behavior for external authentication, authorization, API keys, tokens, and credential handling."*
...
Integration credentials are high-risk. Leaked tokens can expose customer messages, CRM data, and external systems.
...
CLARA should never store raw secrets in normal configuration fields and should use secure credential references managed by backend infrastructure.
...
|---|---|---|
| `credential:read_metadata` | Product action permission | Protected by backend authorization |
| `credential:create` | Product action permission | Protected by backend authorization |
| `credential:rotate` | Product action permission | Protected by backend authorization |
| `credential:revoke` | Product action permission | Protected by backend authorization |
...
MVP must store secrets using environment/secret manager patterns and expose only safe metadata in UI.

---

# Future Behavior

Future versions may support credential rotation, scoped tokens, OAuth reauth flows, secret scanning, and per-connector credential policy.
