...
previous: "165-OAuth-and-Credentials.md"
next: "167-Outbound-Webhooks.md"
---

# Webhook Ingestion

> *"Defines how CLARA receives inbound events from external systems."*
...
Inbound webhooks are untrusted external input. Without validation, attackers or provider errors can create fake messages, duplicate records, or unsafe state changes.
...
CLARA Webhook Ingestion should validate signatures, verify provider identity, normalize payloads, enforce idempotency, and queue processing safely.
...
|---|---|---|
| `webhook:ingest` | Product action permission | Protected by backend authorization |
| `webhook:read` | Product action permission | Protected by backend authorization |
...
MVP must validate webhook payloads for supported providers and prevent duplicate event processing.

---

# Future Behavior

Future versions may support replay, provider-specific diagnostics, dead-letter queue UI, and custom webhook ingestion.
