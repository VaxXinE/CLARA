...
previous: "166-Webhook-Ingestion.md"
next: "168-Email-Channel.md"
---

# Outbound Webhooks

> *"Defines how CLARA sends events to external systems."*
...
Outbound webhooks can leak sensitive data or overload external systems if not governed properly.
...
CLARA Outbound Webhooks should be explicit, scoped, signed, retryable, rate-limited, and auditable.
...
|---|---|---|
| `outbound_webhook:read` | Product action permission | Protected by backend authorization |
| `outbound_webhook:create` | Product action permission | Protected by backend authorization |
| `outbound_webhook:update` | Product action permission | Protected by backend authorization |
| `outbound_webhook:delete` | Product action permission | Protected by backend authorization |
...
MVP may defer outbound webhooks or support limited admin-created webhook subscriptions for safe event types.

---

# Future Behavior

Future versions may support event filtering, signing secret rotation, delivery logs, replay, and webhook analytics.
