...
previous: "163-Channel-vs-Integration-Boundary.md"
next: "165-OAuth-and-Credentials.md"
---

# Connector Lifecycle

> *"Defines how connectors are installed, configured, enabled, disabled, reauthorized, and removed."*
...
External connectors fail, expire, change permissions, or get removed. Users need safe lifecycle controls.
...
CLARA Connector Lifecycle should be explicit and auditable because connectors can move customer data and trigger automation.
...
|---|---|---|
| `connector:read` | Product action permission | Protected by backend authorization |
| `connector:install` | Product action permission | Protected by backend authorization |
| `connector:update` | Product action permission | Protected by backend authorization |
| `connector:disconnect` | Product action permission | Protected by backend authorization |
...
MVP must support enabled/disabled connection status and safe disconnect behavior for supported integrations.

---

# Future Behavior

Future versions may support reauthorization reminders, connector versioning, marketplace review, and policy-based installation.
