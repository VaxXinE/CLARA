...
previous: "88-Ticket-SLA-Policy.md"
next: "90-Ticket-Collaboration.md"
---

# Ticket Status Workflow

> *"Defines status transitions and allowed actions across the ticket workflow."*
...
Without clear status transitions, tickets can be closed too early, reopened inconsistently, or left in unclear states.
...
CLARA Ticket Status Workflow should enforce allowed transitions and prevent invalid state changes.
...
|---|---|---|
| `ticket:update_status` | Product action permission | Protected by backend authorization |
| `ticket:resolve` | Product action permission | Protected by backend authorization |
| `ticket:close` | Product action permission | Protected by backend authorization |
| `ticket:reopen` | Product action permission | Protected by backend authorization |
...
MVP must enforce basic status transitions and require a resolution note when resolving where practical.

---

# Future Behavior

Future versions may support custom workflows, required fields by status, manager approval, and workflow automation.
