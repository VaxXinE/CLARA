...
previous: "146-Workflow-Builder-Experience.md"
next: "148-Workflow-Execution.md"
---

# Workflow Lifecycle

> *"Defines workflow states such as draft, active, paused, archived, and failed."*
...
Automation must be controlled. A workflow should not execute until it is valid, enabled, and scoped.
...
CLARA Workflow Lifecycle should prevent incomplete or unreviewed workflows from running automatically.
...
|---|---|---|
| `workflow:update_status` | Product action permission | Protected by backend authorization |
| `workflow:activate` | Product action permission | Protected by backend authorization |
| `workflow:pause` | Product action permission | Protected by backend authorization |
| `workflow:archive` | Product action permission | Protected by backend authorization |
...
MVP must support enabled/disabled state for automation rules.

---

# Future Behavior

Future versions may support draft/published workflow versions, scheduled activation, approval before activation, and rollback.
