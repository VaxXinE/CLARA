...
previous: "42-Customer-Profile.md"
next: "44-Customer-Timeline.md"
---

# Contact Points

> *"Defines how CLARA stores and uses customer contact methods such as email, phone, WhatsApp, social handles, and external channel IDs."*
...
Customers may interact from multiple channels. Without contact point mapping, CLARA cannot reliably connect messages to customer records.
...
CLARA should model contact points as structured, verified-or-unverified identifiers connected to a Customer.
...
|---|---|---|
| `customer_contact:read` | Product action permission | Protected by backend authorization |
| `customer_contact:create` | Product action permission | Protected by backend authorization |
| `customer_contact:update` | Product action permission | Protected by backend authorization |
...
MVP must support email, phone number, and provider-specific external identifiers for conversation matching.

---

# Future Behavior

Future versions may support contact verification, preferred channel, invalid contact detection, and consent status per contact point.
