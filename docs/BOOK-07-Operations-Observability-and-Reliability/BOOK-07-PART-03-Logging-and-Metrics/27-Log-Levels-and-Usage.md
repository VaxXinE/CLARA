...
previous: "26-Structured-Logging-Standards.md"
next: "28-Log-Event-Taxonomy.md"
project: "CLARA"
---

# Log Levels and Usage

> *"Defines how CLARA should use debug, info, warn, error, and fatal log levels across production systems."*
...
Incorrect log levels create noisy production logs or hide important failure signals.
...
CLARA log levels should be used consistently so logs represent operational significance and not developer mood.
...
| Level | Usage |
|---|---|
| debug | Local/development detail; usually disabled or sampled in production |
| info | Important successful lifecycle events |
| warn | Unexpected but recoverable condition |
| error | Failed operation requiring investigation or user/system impact |
| fatal | Process/service cannot continue safely |
...
```text
info: user logged in, reply sent, job completed
warn: provider retry, validation mismatch, degraded fallback
error: API failed, job exhausted retries, provider unavailable
fatal: app cannot boot, critical config missing
```
...
Do not use `error` for expected validation failures unless they indicate system behavior needing investigation.
