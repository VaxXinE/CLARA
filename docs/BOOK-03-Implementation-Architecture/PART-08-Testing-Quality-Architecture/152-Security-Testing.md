...
previous: "./151-End-to-End-Testing.md"
next: "./153-Performance-Testing.md"
---

# Security Testing

> *"Defines security test implementation for authorization, tenant isolation, injection, XSS, CSRF, SSRF, secrets, dependency scanning, and abuse cases."*
...
This chapter defines how **Security Testing** should be implemented consistently.
...
Athena security testing should be automated in CI/CD and explicitly cover permission-denied, tenant isolation, and abuse-case scenarios.
...
describe("Customer API authorization", () => {
  it("blocks access to another organization's customer", async () => {
    const response = await api
      .asUser(userFromOrgA)
      .get(`/v1/customers/${customerFromOrgB.id}`);
