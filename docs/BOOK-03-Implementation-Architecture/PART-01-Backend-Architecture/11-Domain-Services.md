...
previous: "./README.md"
next: "./12-Application-Services.md"
---

# Domain Services

> *"Defines domain services for business rules that do not naturally belong inside a single entity or value object."*
...
- Prevents entities from becoming overloaded with unrelated logic.
- Keeps complex business policies inside the Domain layer.
- Avoids moving domain rules into application services incorrectly.
- Improves testability for cross-entity domain behavior.
...
```ts
// customer/domain/services/CustomerMergePolicy.ts
export class CustomerMergePolicy {
  canMerge(primary: Customer, duplicate: Customer): boolean {
    ...
    return primary.isActive() && duplicate.isActive();
  }

  merge(primary: Customer, duplicate: Customer): CustomerMerged {
    if (!this.canMerge(primary, duplicate)) {
      throw new DomainError("Customers cannot be merged");
    }

    primary.recordMergedDuplicate(duplicate.id);

    return new CustomerMerged({
