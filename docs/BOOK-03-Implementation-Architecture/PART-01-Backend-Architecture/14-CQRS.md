...
next: "./15-Transactions.md"
---

# CQRS

> *"Defines Command Query Responsibility Segregation for separating write operations from read operations where it improves clarity, performance, and scalability."*
...
- Separates mutation logic from read optimization.
- Prevents read models from corrupting domain write models.
- Supports analytics, search, and dashboard projections.
- Allows performance tuning without compromising domain integrity.
...
```ts
// customer/application/commands/CreateCustomerCommand.ts
...
// customer/application/queries/GetCustomerProfileQuery.ts
...
// customer/application/query-handlers/GetCustomerProfileHandler.ts
export class GetCustomerProfileHandler {
  constructor(
    private readonly authorization: AuthorizationService,
    private readonly readModel: CustomerProfileReadModel,
  ) {}

  async execute(query: GetCustomerProfileQuery): Promise<CustomerProfileDto> {
    await this.authorization.assertCan(query.actor, "customer:read", {
      organizationId: query.organizationId,
      workspaceId: query.workspaceId,
      resourceId: query.customerId,
    });

    return this.readModel.findProfile(query.customerId);
  }
}
