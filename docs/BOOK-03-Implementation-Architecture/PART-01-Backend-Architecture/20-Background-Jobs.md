...
- Keeps request-response paths fast and predictable.
- Improves reliability for external integrations.
- Supports retries and dead-letter handling.
- Makes long-running work observable and recoverable.
...
flowchart TD
    ...
    Boundary --> Runtime[Background Jobs]
...
// notification/jobs/SendNotificationJobHandler.ts
export class SendNotificationJobHandler {
  ...
  async handle(job: Job<SendNotificationPayload>): Promise<void> {
    const alreadyProcessed = await this.idempotencyStore.exists(job.idempotencyKey);

    if (alreadyProcessed) {
      return;
    }

    await this.notificationProvider.send(job.payload);

    await this.idempotencyStore.markProcessed(job.idempotencyKey);

    this.logger.info("Notification job processed", {
      jobName: job.name,
