...
previous: "./34-Forms-Validation.md"
next: "./36-Authentication-Session.md"
---

# API Client

> *"Defines frontend API client architecture, request handling, typed responses, retries, and error mapping."*
...
This chapter defines how **API Client** should be implemented consistently across Athena client applications.
...
Athena frontend should use typed API clients and never call raw HTTP directly from UI widgets.
...
class CustomerApiClient {
  ...
  CustomerApiClient(this.http);

  Future<List<CustomerDto>> listCustomers() async {
    final response = await http.get('/v1/customers');

    return (response.data as List)
        .map((item) => CustomerDto.fromJson(item))
        .toList();
  }
}
