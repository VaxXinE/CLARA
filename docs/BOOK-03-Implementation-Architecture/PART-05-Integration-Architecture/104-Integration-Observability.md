...
previous: "./103-Data-Mapping-Transformation.md"
next: "./105-Integration-Security-Summary.md"
---

# Integration Observability

> *"Defines logs, metrics, traces, delivery status, provider latency, failure rate, and integration health dashboards."*
...
This chapter defines how **Integration Observability** should be implemented safely and consistently.
...
Athena integrations should emit structured telemetry for every external request, webhook, retry, failure, and provider status change.
...
```ts
// integration/observability/IntegrationTelemetry.ts
export class IntegrationTelemetry {
  recordExternalCall(event: ExternalCallTelemetry): void {
    metrics.histogram("integration.external_call.latency_ms", event.latencyMs);
    metrics.counter("integration.external_call.count", 1, {
      provider: event.provider,
      action: event.action,
      status: event.status,
    });

    logger.info("External integration call completed", {
