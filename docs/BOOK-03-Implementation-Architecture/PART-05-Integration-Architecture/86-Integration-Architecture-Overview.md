...
previous: "./README.md"
next: "./87-REST-API-Implementation.md"
---
...
```ts
// integration/application/IntegrationOrchestrator.ts
export class IntegrationOrchestrator {
  ...
  async execute(input: IntegrationExecutionInput): Promise<IntegrationExecutionResult> {
    await this.policy.assertAllowed(input.actor, input.connectorKey, input.action);

    const connector = this.registry.get(input.connectorKey);
    const result = await connector.execute(input.action, input.payload);

    await this.audit.record({
      action: "integration.executed",
      actorId: input.actor.id,
      resourceId: input.connectorKey,
