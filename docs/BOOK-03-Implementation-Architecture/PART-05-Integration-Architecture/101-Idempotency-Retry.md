...
previous: "./100-Rate-Limit-Quota.md"
next: "./102-Event-Driven-Integration.md"
---

# Idempotency Retry

> *"Defines idempotency keys, safe retries, exponential backoff, duplicate prevention, and retry exhaustion handling."*
...
This chapter defines how **Idempotency Retry** should be implemented safely and consistently.
...
Athena integration writes and webhook processing should be idempotent and retry-safe by default.
...
```ts
// integration/reliability/IdempotentExecutor.ts
export class IdempotentExecutor {
  constructor(private readonly store: IdempotencyStore) {}

  async execute<T>(key: string, operation: () => Promise<T>): Promise<T> {
    const existing = await this.store.findResult<T>(key);
    ...
    await this.store.saveResult(key, result);
