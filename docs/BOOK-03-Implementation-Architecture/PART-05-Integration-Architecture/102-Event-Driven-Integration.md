...
next: "./103-Data-Mapping-Transformation.md"
---

# Event Driven Integration

> *"Defines event-driven integration patterns, event consumers, outbox, inbox, delivery guarantees, and replay behavior."*
...
```ts
// integration/events/ExternalEventConsumer.ts
export class ExternalEventConsumer {
  ...
  async consume(event: ExternalEvent): Promise<void> {
    const isDuplicate = await this.inbox.hasProcessed(event.provider, event.eventId);
    ...
    await this.inbox.markProcessed(event.provider, event.eventId);
  }
}
