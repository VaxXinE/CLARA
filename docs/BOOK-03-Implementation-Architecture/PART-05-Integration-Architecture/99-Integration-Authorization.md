...
next: "./100-Rate-Limit-Quota.md"
---

# Integration Authorization

> *"Defines authorization checks for integration actions, connector access, tool execution, external data exposure, and delegated permissions."*
...
```ts
// integration/security/IntegrationAuthorizationService.ts
export class IntegrationAuthorizationService {
  constructor(private readonly authorization: AuthorizationService) {}

  async assertCanExecute(input: IntegrationAuthorizationInput): Promise<void> {
    await this.authorization.assertCan(input.actor, `integration:${input.connectorKey}:${input.action}`, {
