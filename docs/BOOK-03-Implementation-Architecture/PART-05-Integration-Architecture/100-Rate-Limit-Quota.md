...
next: "./101-Idempotency-Retry.md"
---

# Rate Limit Quota

> *"Defines integration rate limits, quotas, provider limits, tenant limits, backpressure, and abuse prevention."*
...
```ts
// integration/policy/IntegrationRateLimiter.ts
export class IntegrationRateLimiter {
  constructor(private readonly limiter: RateLimiter) {}

  async assertAllowed(input: IntegrationRateLimitInput): Promise<void> {
    const key = [
      "integration",
    ...
    if (!allowed) {
      throw new TooManyRequestsError("Integration rate limit exceeded");
    }
  }
}
