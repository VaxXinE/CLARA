...
previous: "./89-Webhook-Implementation.md"
next: "./91-OAuth-Implementation.md"
---

# WebSocket Realtime

> *"Defines realtime communication, WebSocket sessions, event subscriptions, presence, tenant scoping, and connection security."*
...
This chapter defines how **WebSocket Realtime** should be implemented safely and consistently.
...
Athena realtime channels should be authenticated, tenant-scoped, permission-aware, and backed by server-side event authorization.
...
```ts
// realtime/application/RealtimeSubscriptionService.ts
export class RealtimeSubscriptionService {
  constructor(private readonly authorization: AuthorizationService) {}

  async subscribe(input: SubscribeInput): Promise<Subscription> {
    await this.authorization.assertCan(input.actor, "realtime:subscribe", {
    ...
    return {
      channel: buildTenantChannel(input.organizationId, input.workspaceId, input.channel),
    };
  }
}
