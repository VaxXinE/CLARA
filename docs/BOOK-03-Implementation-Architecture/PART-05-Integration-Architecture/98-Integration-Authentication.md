...
previous: "./97-Third-Party-API-Clients.md"
next: "./99-Integration-Authorization.md"
---

# Integration Authentication

> *"Defines authentication patterns for external integrations including API keys, OAuth tokens, signed requests, and service credentials."*
...
This chapter defines how **Integration Authentication** should be implemented safely and consistently.
...
Athena integration credentials should be stored in a secure token vault and accessed only through credential provider interfaces.
...
```ts
// integration/security/CredentialProvider.ts
export interface CredentialProvider {
  getCredential(input: CredentialLookupInput): Promise<IntegrationCredential>;
}

export class VaultCredentialProvider implements CredentialProvider {
  async getCredential(input: CredentialLookupInput): Promise<IntegrationCredential> {
    return vault.read({
      path: `integrations/${input.organizationId}/${input.provider}`,
    });
  }
}
