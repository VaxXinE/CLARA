...
previous: "./131-Tenant-Isolation.md"
next: "./133-Encryption-In-Transit.md"
---

# Zero Trust Implementation

> *"Defines zero-trust principles for service-to-service access, user access, device/context awareness, and least privilege."*
...
This chapter defines how **Zero Trust Implementation** should be implemented safely and consistently.
...
Athena should trust no request by default and require explicit identity, authorization, scope, and telemetry for sensitive actions.
...
```ts
// security/zero-trust/RequestTrustEvaluator.ts
export class RequestTrustEvaluator {
  evaluate(input: TrustEvaluationInput): TrustDecision {
    return {
      authenticated: Boolean(input.actor),
      hasTenantScope: Boolean(input.organizationId),
      hasCorrelationId: Boolean(input.correlationId),
      riskLevel: input.ipReputation === "bad" ? "high" : "normal",
    };
  }
}
