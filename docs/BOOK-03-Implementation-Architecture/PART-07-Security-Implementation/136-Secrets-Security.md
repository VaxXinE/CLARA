...
previous: "./135-Key-Management.md"
next: "./137-Input-Validation-Sanitization.md"
---

# Secrets Security

> *"Defines secret storage, secret rotation, access boundaries, runtime injection, leak prevention, and emergency revocation."*
...
This chapter defines how **Secrets Security** should be implemented safely and consistently.
...
Athena secrets should never be committed to source code and must be accessed through a managed secret provider with least privilege.
...
```ts
// security/secrets/SecretAccessService.ts
export class SecretAccessService {
  constructor(private readonly provider: SecretProvider) {}

  async getIntegrationSecret(input: SecretRequest): Promise<string> {
    if (!input.actor.serviceAccount.permissions.includes("secret:read")) {
      throw new AuthorizationError("Service cannot read secrets");
    }

    return this.provider.getSecret(`athena/${input.environment}/${input.secretName}`);
  }
}
