...
previous: "./133-Encryption-In-Transit.md"
next: "./135-Key-Management.md"
---

# Encryption At Rest

> *"Defines data-at-rest encryption for databases, object storage, backups, logs, and sensitive derived stores."*
...
This chapter defines how **Encryption At Rest** should be implemented safely and consistently.
...
Athena should encrypt sensitive data at rest using managed encryption where possible and field-level encryption for highly sensitive values.
...
```ts
// security/crypto/FieldEncryptionService.ts
export class FieldEncryptionService {
  constructor(private readonly keyProvider: KeyProvider) {}

  async encryptPii(value: string, context: EncryptionContext): Promise<EncryptedValue> {
    const key = await this.keyProvider.getKey("pii-field-key", context);

    return encryptWithAead({
      plaintext: value,
      key,
      associatedData: context.organizationId,
    });
  }
}
