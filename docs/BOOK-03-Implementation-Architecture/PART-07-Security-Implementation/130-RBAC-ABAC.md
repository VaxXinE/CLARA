...
```ts
// security/policy/PolicyEngine.ts
export class PolicyEngine {
  evaluate(input: PolicyEvaluationInput): PolicyDecision {
    const roleAllowed = input.actor.roles.some((role) =>
      role.permissions.includes(input.permission),
    );

    const tenantAllowed =
      input.actor.organizationId === input.resource.organizationId;

    const workspaceAllowed =
      !input.resource.workspaceId ||
      input.actor.workspaceIds.includes(input.resource.workspaceId);

    return {
      allowed: roleAllowed && tenantAllowed && workspaceAllowed,
    };
  }
}
