...
previous: "./134-Encryption-At-Rest.md"
next: "./136-Secrets-Security.md"
---

# Key Management

> *"Defines cryptographic key lifecycle, key rotation, key access control, envelope encryption, and key auditability."*
...
This chapter defines how **Key Management** should be implemented safely and consistently.
...
Athena cryptographic keys should be managed through a dedicated key management system with rotation, access control, and audit logs.
...
```ts
// security/keys/KeyProvider.ts
export interface KeyProvider {
  getKey(keyAlias: string, context: KeyContext): Promise<CryptoKeyRef>;
  rotateKey(keyAlias: string): Promise<void>;
}

export type KeyContext = {
  organizationId?: string;
  purpose: "pii" | "token" | "backup" | "audit";
};
