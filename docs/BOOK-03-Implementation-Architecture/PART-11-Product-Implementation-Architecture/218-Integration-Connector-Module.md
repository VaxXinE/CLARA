...
chapter: "218"
title: "Integration Connector Module"
version: "1.0.0"
status: "official"
owner: "Athena Product Architecture Team"
last_updated: "2026-07-07"
classification: "implementation-architecture"
previous: "./217-AI-Assistant-Product-Module.md"
next: "./219-Billing-Subscription-Module.md"
---

# Integration Connector Module

> *"Defines implementation architecture for product-facing connector management, connection status, credential binding, provider actions, and sync state."*
...
This chapter defines how **Integration Connector Module** should be implemented consistently.
...
Athena Integration Connector product module should expose connection management while delegating provider execution to Integration Architecture adapters.
...
```ts
// connector/application/use-cases/ConnectProviderUseCase.ts
export class ConnectProviderUseCase {
  constructor(
    private readonly authorization: AuthorizationService,
    private readonly oauth: OAuthConnectionService,
    private readonly audit: AuditService,
  ) {}

  async execute(input: ConnectProviderInput): Promise<ConnectProviderOutput> {
    await this.authorization.assertCan(input.actor, "integration:connect", {
      organizationId: input.organizationId,
      workspaceId: input.workspaceId,
      resourceId: input.provider,
    });

    const redirectUrl = await this.oauth.createAuthorizationUrl(input.provider, {
      organizationId: input.organizationId,
      workspaceId: input.workspaceId,
    });
