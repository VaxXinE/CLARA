...
previous: "./215-Workflow-Automation-Module.md"
next: "./217-AI-Assistant-Product-Module.md"
---

# Notification Module

> *"Defines implementation architecture for in-app notifications, email, push, webhooks, preferences, delivery state, and retry behavior."*
...
This chapter defines how **Notification Module** should be implemented consistently.
...
Athena Notification should be implemented as asynchronous delivery with user preferences, idempotency, provider abstraction, and delivery logs.
...
```ts
// notification/application/jobs/SendNotificationJob.ts
export class SendNotificationJobHandler {
  constructor(
  ...
  async handle(job: SendNotificationJob): Promise<void> {
    const allowed = await this.preferences.canSend(job.recipientId, job.channel);
