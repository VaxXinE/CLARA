...
next: "./215-Workflow-Automation-Module.md"
---

# Knowledge Base Module

> *"Defines implementation architecture for articles, document ingestion, publishing, versioning, access control, search, and RAG grounding."*
...
```ts
// knowledge/application/use-cases/PublishArticleUseCase.ts
export class PublishArticleUseCase {
  ...
  async execute(input: PublishArticleInput): Promise<void> {
    await this.authorization.assertCan(input.actor, "knowledge:publish", {
    ...
    await this.indexer.reindexArticle(article.id);
  }
}
```
