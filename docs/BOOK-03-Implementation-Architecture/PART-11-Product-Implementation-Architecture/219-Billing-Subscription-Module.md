...
next: "./220-Admin-Console-Module.md"
---

# Billing Subscription Module

> *"Defines implementation architecture for plans, subscriptions, invoices, usage tracking, entitlements, quotas, and billing events."*
...
```ts
// billing/application/EntitlementService.ts
export class EntitlementService {
  constructor(private readonly subscriptionRepository: SubscriptionRepository) {}

  async assertEntitled(input: EntitlementCheckInput): Promise<void> {
    const subscription = await this.subscriptionRepository.findActiveByOrganization(
      input.organizationId,
    );

    if (!subscription.allows(input.featureKey)) {
      throw new EntitlementError("Feature is not available for current plan");
    }
  }
}
```
