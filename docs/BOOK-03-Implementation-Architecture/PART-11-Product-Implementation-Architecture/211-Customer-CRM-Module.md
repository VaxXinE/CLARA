...
next: "./212-Conversation-Inbox-Module.md"
---

# Customer CRM Module

> *"Defines implementation architecture for customer profile, lifecycle, CRM metadata, contact points, segmentation, and timeline."*
...
export class Customer {
  private timeline: CustomerTimelineEvent[] = [];
  ...
  static create(input: CreateCustomerProps): Customer {
    return new Customer({
      ...
      displayName: CustomerName.create(input.displayName),
      status: "active",
    });
  }

  addTimelineEvent(event: CustomerTimelineEvent): void {
    if (this.status === "archived") {
      throw new DomainError("Archived customer cannot be modified");
    }
