...
next: "./223-Settings-Configuration-Module.md"
---

# Audit Compliance Module

> *"Defines implementation architecture for audit viewer, compliance exports, evidence collection, access review, and investigation workflows."*
...
```ts
// compliance/application/queries/SearchAuditLogsQueryHandler.ts
export class SearchAuditLogsQueryHandler {
  constructor(
    private readonly authorization: AuthorizationService,
    private readonly auditReadModel: AuditReadModel,
  ) {}

  async execute(query: SearchAuditLogsQuery): Promise<AuditSearchResult> {
    await this.authorization.assertCan(query.actor, "audit:read", {
      organizationId: query.organizationId,
      workspaceId: query.workspaceId,
    });

    return this.auditReadModel.search({
      organizationId: query.organizationId,
      workspaceId: query.workspaceId,
