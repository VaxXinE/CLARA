...
next: "./221-Analytics-Reporting-Module.md"
---

# Admin Console Module

> *"Defines implementation architecture for admin console capabilities, internal operations, elevated actions, support tools, and operational safeguards."*
...
```ts
// admin/application/use-cases/ImpersonationRequestUseCase.ts
export class ImpersonationRequestUseCase {
  constructor(
    private readonly authorization: AuthorizationService,
    private readonly audit: AuditService,
  ) {}

  async execute(input: ImpersonationRequestInput): Promise<void> {
    await this.authorization.assertCan(input.actor, "admin:impersonation:request", {
      organizationId: input.targetOrganizationId,
      resourceId: input.targetUserId,
    });

    await this.audit.record({
      action: "admin.impersonation.requested",
