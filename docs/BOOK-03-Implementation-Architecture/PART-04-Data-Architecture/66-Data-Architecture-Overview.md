...
This chapter defines how **Data Architecture Overview** should be implemented safely and consistently.
...
```ts
// data/application/DataAccessPolicy.ts
export class DataAccessPolicy {
  assertTenantScope(input: TenantScopedRequest): void {
    if (!input.organizationId || !input.workspaceId) {
      throw new SecurityError("Missing tenant scope");
    }
  }

  assertDataClassificationAllowed(classification: DataClassification): void {
    if (classification === "secret") {
      throw new SecurityError("Secret data must not be accessed through generic data paths");
    }
  }
}
