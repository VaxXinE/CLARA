...
next: "./61-Rate-Limits.md"
---

# Cost Control

> *"Defines AI budget controls, token limits, model selection, caching, quotas, and usage reporting."*
...
```ts
// ai/application/policy/AiBudgetPolicy.ts
export class AiBudgetPolicy {
  constructor(private readonly usageRepository: AiUsageRepository) {}

  async assertWithinBudget(input: AiBudgetCheckInput): Promise<void> {
    const usage = await this.usageRepository.getMonthlyUsage(input.organizationId);

    if (usage.estimatedCostUsd >= input.monthlyBudgetUsd) {
      throw new LimitExceededError("AI budget exceeded");
    }
  }
}
