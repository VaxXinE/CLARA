...
```ts
// ai/application/context/ContextBuilder.ts
export class ContextBuilder {
  ...
  async build(input: BuildContextInput): Promise<AiContext> {
    await this.permissionService.assertCan(input.actor, "knowledge:read", {
      organizationId: input.organizationId,
      workspaceId: input.workspaceId,
    });

    const documents = await this.knowledgeSearch.search({
      query: input.query,
      organizationId: input.organizationId,
      workspaceId: input.workspaceId,
      limit: 8,
