...
next: "./50-Context-Engineering.md"
---

# Prompt Architecture

> *"Defines prompt templates, system instructions, prompt versioning, prompt variables, and prompt safety boundaries."*
...
```ts
// ai/domain/prompts/PromptTemplate.ts
export class PromptTemplate {
  ...
  render(variables: Record<string, string>): RenderedPrompt {
    return {
      system: this.system,
      user: this.userTemplate.replace(/{{(\w+)}}/g, (_, key) => {
