...
next: "./56-AI-Workflow-Orchestration.md"
---

# Agent Architecture

> *"Defines AI agent boundaries, planning, tool usage, state handling, and safe execution lifecycle."*
...
```ts
// ai/application/agents/AgentRunner.ts
export class AgentRunner {
  ...
  async run(input: AgentRunInput): Promise<AgentRunResult> {
    await this.policy.assertAllowed(input.actor, input.agentId);

    const plan = await this.planner.createPlan(input.goal);

    for (const step of plan.steps) {
      await this.policy.assertStepAllowed(step);
      await this.toolExecutor.execute(step.toolCall);
    }
