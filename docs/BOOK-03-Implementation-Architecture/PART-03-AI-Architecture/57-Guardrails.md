...
next: "./58-AI-Evaluation.md"
---

# Guardrails

> *"Defines input guardrails, output guardrails, policy enforcement, blocked content, and safe completion behavior."*
...
```ts
// ai/application/guardrails/AiGuardrailService.ts
export class AiGuardrailService {
  async validateRequest(request: AiCompletionRequest): Promise<AiCompletionRequest> {
    if (request.messages.some((message) => containsSecretPattern(message.content))) {
      throw new SecurityError("AI request contains sensitive secret-like content");
    }

    return request;
  }

  async validateResponse(result: AiCompletionResult): Promise<AiCompletionResult> {
    return {
      ...result,
      content: redactSensitiveOutput(result.content),
    };
  }
}
