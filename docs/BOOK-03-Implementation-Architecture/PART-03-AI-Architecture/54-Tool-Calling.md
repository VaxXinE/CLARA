...
next: "./55-Agent-Architecture.md"
---

# Tool Calling

> *"Defines AI tool execution, tool schemas, authorization, sandboxing, confirmation, and audit."*
...
```ts
// ai/application/tools/ToolExecutor.ts
export class ToolExecutor {
  ...
  async execute(input: ToolExecutionInput): Promise<ToolExecutionResult> {
    ...
    await this.authorization.assertCan(input.actor, tool.permission, {
      organizationId: input.organizationId,
      workspaceId: input.workspaceId,
    });

    const validatedArgs = tool.schema.parse(input.args);
