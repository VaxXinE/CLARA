...
previous: "./46-AI-Architecture-Overview.md"
next: "./48-Model-Provider-Abstraction.md"
---

# AI Gateway

> *"Defines the central AI gateway for request normalization, policy enforcement, routing, retries, and response handling."*
...
This chapter defines how **AI Gateway** should be implemented safely and consistently.
...
Athena backend should route all model requests through a centralized AI Gateway.
...
```ts
// ai/application/ports/AiGateway.ts
export interface AiGateway {
  complete(request: AiCompletionRequest): Promise<AiCompletionResult>;
}

// ai/infrastructure/gateway/DefaultAiGateway.ts
export class DefaultAiGateway implements AiGateway {
  ...
  async complete(request: AiCompletionRequest): Promise<AiCompletionResult> {
    const safeRequest = await this.guardrails.validateRequest(request);
    const provider = await this.router.selectProvider(safeRequest.modelPolicy);

    const result = await provider.complete(safeRequest);

    return this.guardrails.validateResponse(result);
  }
}
