...
next: "./59-AI-Observability.md"
---

# AI Evaluation

> *"Defines offline evaluation, regression testing, golden datasets, scoring, and release gates for AI behavior."*
...
```ts
// ai/evaluation/AiEvaluationRunner.ts
export class AiEvaluationRunner {
  ...
  async run(dataset: EvaluationDataset): Promise<EvaluationReport> {
    ...
    for (const item of dataset.items) {
      const output = await this.gateway.complete(item.request);
      results.push(scoreOutput(item.expected, output));
    }
