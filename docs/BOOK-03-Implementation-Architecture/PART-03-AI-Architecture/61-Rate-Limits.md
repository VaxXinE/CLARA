...
previous: "./60-Cost-Control.md"
next: "./62-Human-In-The-Loop.md"
---

# Rate Limits

> *"Defines AI rate limiting, abuse prevention, quota boundaries, and burst control."*
...
This chapter defines how **Rate Limits** should be implemented safely and consistently.
...
Athena AI endpoints should enforce rate limits per organization, workspace, actor, capability, and provider policy.
...
```ts
// ai/application/policy/AiRateLimitService.ts
export class AiRateLimitService {
  constructor(private readonly limiter: RateLimiter) {}

  async assertAllowed(input: AiRateLimitInput): Promise<void> {
    const key = [
      "ai",
    ...
    const allowed = await this.limiter.consume(key, 1);

    if (!allowed) {
      throw new TooManyRequestsError("AI rate limit exceeded");
    }
  }
}
