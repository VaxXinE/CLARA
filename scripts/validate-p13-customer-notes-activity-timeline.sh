...
EXPECTED_BRANCH="feat/p13-customer-notes-activity-timeline"
...
required_tests=(
  "services/api/tests/p13-customer-notes-route.test.ts"
  "services/api/tests/p13-customer-notes-service.test.ts"
  "services/api/tests/p13-customer-notes-workspace-boundary.test.ts"
  "services/api/tests/p13-customer-notes-input-validation.test.ts"
  "services/api/tests/p13-customer-activity-timeline-route.test.ts"
  "services/api/tests/p13-customer-activity-timeline-service.test.ts"
  "services/api/tests/p13-customer-activity-timeline-security.test.ts"
  "services/api/tests/p13-customer-notes-no-billing-side-effect.test.ts"
  "apps/dashboard/src/components/p13-customer-notes-activity-timeline.test.tsx"
  "apps/dashboard/src/components/p13-customer-notes-activity-security.test.tsx"
  "apps/extension/src/tests/p13-customer-notes-extension-boundary.test.ts"
)

for path in "${required_docs[@]}" "${required_tests[@]}"; do
  if [[ ! -f "$ROOT_DIR/$path" ]]; then
    echo "Missing required P13-PR-02 artifact: $path" >&2
    exit 1
  fi
done
...
docs_blob="$(
  cat \
    ...
    "$ROOT_DIR/docs/product/CLARA-P13-CUSTOMER-NOTES-ACTIVITY-TIMELINE.md" \
...
required_phrases=(
  ...
  "P13-PR-02 is current"
  "internal CRM usage is the focus"
  "billing/payment is deferred"
  "notes/timeline are workspace-scoped internal CRM features"
  "CLARA is not production deployed yet"
  "CLARA is not public GA launched yet"
  "Timeline must not expose raw provider/audit/secrets"
)

for phrase in "${required_phrases[@]}"; do
  if ! grep -Fq "$phrase" <<<"$docs_blob"; then
    echo "Missing required P13-PR-02 doc phrase: $phrase" >&2
    exit 1
  fi
done
...
echo "CLARA P13-PR-02 VALIDATION PASSED"
