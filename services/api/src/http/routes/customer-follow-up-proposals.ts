...
} from "../../customers/customer-follow-up-proposal-policy";
import { CustomerFollowUpProposalService } from "../../customers/customer-follow-up-proposal-service";
import type { ReviewCustomerFollowUpProposalInput } from "../../customers/customer-follow-up-proposal-types";
...
const bodySchema = z.object({
  source: z.enum(customerFollowUpProposalSources),
  proposalIntent: z.enum(customerFollowUpProposalIntents),
...
export async function registerCustomerFollowUpProposalRoutes(
  app: FastifyInstance,
  authProvider: AuthProvider,
  service: CustomerFollowUpProposalService,
): Promise<void> {
  app.post(
    "/api/v1/customers/:customer_id/follow-up-proposals/review",
    {
    ...
    async (request) => {
      ...
      const input: ReviewCustomerFollowUpProposalInput = {
        auth,
        customerId: parseCustomerId(params.customer_id ?? ""),
        source: body.source,
        proposalIntent: body.proposalIntent,
      };
      ...
      return service.reviewFollowUpProposal(input);
    },
  );
}
