...
import { ObservabilitySloAlertReadinessService } from "../../reliability/observability-readiness-service";
  ...
  for (const key of scopeKeys) {
    if (key in query) {
      throw new ValidationError(
        "Invalid observability SLO alert readiness query.",
        [
          {
            path: `query.${key}`,
...
export async function registerReliabilityObservabilitySloAlertReadinessRoutes(
  app: FastifyInstance,
  authProvider: AuthProvider,
  service = new ObservabilitySloAlertReadinessService(),
): Promise<void> {
  app.get(
