...
describe("GET /api/v1/billing/plan-entitlement/readiness", () => {
  it("requires auth and returns safe P11 billing readiness", async () => {
    const app = await createServer({ env: testEnv });

    const unauthenticated = await app.inject({
      method: "GET",
      url: "/api/v1/billing/plan-entitlement/readiness",
    });
  ...
  it("rejects client supplied workspace scope", async () => {
    ...
    const response = await app.inject({
      method: "GET",
      url: "/api/v1/billing/plan-entitlement/readiness?workspaceId=wks_other",
      headers: ownerHeaders,
    });
