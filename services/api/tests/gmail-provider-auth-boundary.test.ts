...
describe("Gmail provider config boundary", () => {
  ...
  it("loads Gmail provider config safely from env-like input", () => {
    const config = loadGmailProviderConfig({
      ...
      GMAIL_INBOUND_SYNC_SCHEDULER_ENABLED: "true",
      GMAIL_INBOUND_SYNC_SCHEDULER_INTERVAL_MS: "120000",
      GMAIL_INBOUND_SYNC_SCHEDULER_MAX_ACCOUNTS_PER_TICK: "5",
      GMAIL_INBOUND_SYNC_SCHEDULER_MAX_MESSAGES_PER_ACCOUNT: "7",
    ...
    expect(config).toEqual({
      ...
      apiMode: "disabled",
      apiTimeoutMs: 12000,
      inboundSyncSchedulerEnabled: true,
      inboundSyncSchedulerIntervalMs: 120000,
      inboundSyncSchedulerMaxAccountsPerTick: 5,
      inboundSyncSchedulerMaxMessagesPerAccount: 7,
