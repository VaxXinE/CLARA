...
describe("GET /api/v1/enterprise/session-policy/readiness", () => {
  it("requires auth, rejects client workspace authority, and returns safe readiness", async () => {
    const app = await createServer({ env: testEnv });

    const unauthenticated = await app.inject({
      method: "GET",
      url: "/api/v1/enterprise/session-policy/readiness",
    });
    const spoofed = await app.inject({
      method: "GET",
      url: "/api/v1/enterprise/session-policy/readiness?workspace_id=wks_other",
