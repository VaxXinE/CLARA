...
describe("GET /api/v1/reliability/observability-slo-alert/readiness", () => {
  it("requires auth and returns safe P11 readiness", async () => {
    const app = await createServer({ env: testEnv });

    const unauthenticated = await app.inject({
      method: "GET",
      url: "/api/v1/reliability/observability-slo-alert/readiness",
    });
  ...
  it("rejects client supplied workspace scope", async () => {
    ...
    const response = await app.inject({
      method: "GET",
      url: "/api/v1/reliability/observability-slo-alert/readiness?workspaceId=wks_other",
      headers: ownerHeaders,
    });
